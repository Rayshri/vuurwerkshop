...
<body>
    ...
    <div class="container">
        <div class="row">
            ...
            <h2 class="page-header col-sm-9 col-sm-offset-1">Vuurwerk</h2>
            <br>
            <div class="rec-list col-sm-9 col-sm-offset-1">
                <?php
                if($_GET['cat'] == 'sier') {
                    $query = $db->prepare("SELECT * FROM products INNER JOIN categorie ON products.category = categorie.id AND category = 1");
                }
                    else if($_GET['cat'] == 'knal') {
                        $query = $db->prepare("SELECT * FROM products INNER JOIN categorie ON products.category = categorie.id AND category = 2");
                    } 
                    else if($_GET['cat'] == 'ovz'){
                        $query = $db->prepare("SELECT * FROM products INNER JOIN categorie ON products.category = categorie.id");
                    }
                    $query->execute();
                    $result = $query->fetchAll(PDO::FETCH_ASSOC);
                    foreach($result as &$data) {
                        echo "<div style='float: left; width: 49%; max-height: 250px; background-color: rgba(19, 19, 19, 0.671); margin: 2px; padding-left: 10px;'";
                            echo $data["id"] . "<br>";
